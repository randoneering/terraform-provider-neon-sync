...
- **Porting SDK v2 validators for free-text fields.** Don't carry over SDK v2 validators on cron expressions, slugs, prefixes, or other free-text attributes. The server validates these and returns clearer errors than a client-side port can. Only port validators that constrain union discriminators or enum values where the SDK exposes an explicit constructor. See `adding-neon-resource` Common Mistakes for the full rationale.
